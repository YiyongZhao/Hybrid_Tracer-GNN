"""_non_hybrid_sim_and_verify.py_
...
from simulation_models import no_hybrid
import numpy as np
import tskit
...
current_folder = '0907_non_hybrid'
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Options for non_hybrid_simulation_and_verify.py", add_help=True)
    ...
    print("The current simulation is Non-Hybrid data.", flush=True)
    ...
    cu_part = f"_cu{cu}_"
    df_config = pd.DataFrame(columns=['time_units', 'length', 't1',
                                      't2', 't3', 'recomb_rate', 'mutate_rate', 'Num_of_samples',
                                      'Num_of_sites', "Num_of_trees", "Run_name"])
    for num in range(run_nums):
        num_part = f"{num+1}"
        try:
            config_list, ts = no_hybrid(coal_units=cu)
    ...
    print(f"The simulation of {run_nums} {cu_part} non-hybrid data is done.\n")
