...
class HybGNNTrainer(object):
    ...
    def score(self):
        ...
        for graph_pair in tqdm(self.testing_graphs):      
            ...
            if (math.log(target) * gedfactor-math.log(prediction) * gedfactor)**2 < 1:
                self.precision.append(1)

            else:
                self.precision.append(0)
            if (-math.log(prediction) * gedfactor-2)**2 < 1:
                self.hyb.append(1)
            if (-math.log(prediction) * gedfactor-4)**2 < 1:
                self.admix.append(1)
            if (-math.log(prediction) * gedfactor-8)**2 < 1:
                self.nohyb.append(1)
            if (-math.log(prediction) * gedfactor-6)**2 < 1:
                self.agf.append(1)

        print("ACC:", np.mean(self.precision))
        print("ACC-hyb:", len(self.hyb))
        print("ACC-admix:", len(self.admix))
        print("ACC-nohyb:", len(self.nohyb))
        print("ACC-admix with gene flow:", len(self.agf))
